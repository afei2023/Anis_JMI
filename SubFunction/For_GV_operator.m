function Output = For_GV_operator(qu, velocity_at_depth, velocity_nmo_at_depth, velocity_horizontal_at_depth, ...
    propagation_taper, omega, kx, num_x, num_x2, dx, dz, num_freq_bins)
% FOR_GV_OPERATOR: Forward propagation gradient operator for anisotropic media
%
% Inputs:
%   qu                           : Input wavefield at current step
...
% Expand dimensions for 3D computation
...
%% Step 2: Build Forward Propagation Operator G
...
% Vertical wavenumber kz with anisotropy
kz_3D = conj(sqrt(C_ps .* (A_ps ./ B_ps)));

% Anisotropic correction term D_ani
stabilizer = 0.05; % Stabilization factor
...
% Forward propagation operator Gkx in frequency domain
...
Output = zeros(size(qu));

parfor ifr = 1:num_freq_bins
    wavefield_slice = squeeze(qu(:, :, ifr));
