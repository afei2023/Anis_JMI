function G_BetaH = make_GBetaH(Res, Q_up, velocity, velocity_nmo, velocity_horizontal, ...
...
% Initialize output
G_BetaH = zeros(num_x, num_z); % Gradient model (BetaV)
...
for iz = 1:num_z
    ...
    dq_extrap = Back_GH_operator(Res_wavefield, velocity_at_depth, velocity_nmo_at_depth, ...
    ...
    G_BetaH(:, iz) = sum(sum(4 * real(dq_extrap .* conj(Q_slice)), 2), 3);  % G*E-*Q-=dBeta-
